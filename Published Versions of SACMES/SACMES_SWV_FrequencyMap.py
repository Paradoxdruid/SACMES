...
def _retrieve_file(file, electrode, frequency):
    ...
    try:
        if e_var == "single":
            ...
            filename2 = "E%s_%s%sHz_%d.txt" % (
                electrode,
                handle_variable,
                frequency,
                file,
            )
            filename3 = "E%s_%s%sHz__%d.txt" % (
                electrode,
                handle_variable,
                frequency,
                file,
...
class InputFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.SingleElectrodeFile = ttk.Button(
            self.ElectrodeListboxFrame,
            text="Multipotentiostat",
            style="On.TButton",
            command=lambda: self.ElectrodeSelect("Multipotentiostat"),
        )
        ...
        self.SaveBox = Checkbutton(
            ...
            offvalue=False,
            text="Export Data",
        ).grid(row=17, column=0, columnspan=4)
    ...
    def __init__(self, parent, controller):
        ...
        for electrode in electrode_list:
            ...
            electrode_label = tk.Label(
                self.electrode_frame, text="E%s" % electrode, font=HUGE_FONT
            )
            ...
            for figure in figures:
                ...
                anim.append(
                    ElectrochemicalAnimation(
                        fig, self.ax, electrode, resize_interval=None, fargs=None
                    )
                )
                fig_count += 1
