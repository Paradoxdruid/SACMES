...
class RealTimeAnalysis(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        container = tk.Frame(self, relief="flat", bd=5).grid(
            row=0, column=0, padx=5, sticky="nsw"
        )
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        ImportFileLabel = tk.Label(self, text="Data File Handle", font=LARGE_FONT).grid(
        ...
        point_limit_label = tk.Label(
            self, text="Enter Number of Points to Average", font=LARGE_FONT
        )
        ...
        start_button = ttk.Button(
            self, text="Initiate Visualization", command=self.StartProgram
        ).grid(row=25, column=0, columnspan=3, pady=5, padx=5)
        quit_button = tk.Button(self, text="Quit", command=lambda: quit()).grid(
            row=30, column=0, columnspan=3, pady=5, padx=5
        )
        ...
        self.SaveBox = Checkbutton(
            ...
            offvalue=False,
            text="Export Data",
        ).grid(row=35, column=0, columnspan=3)
    ...
    def StartProgram(self):
        ...
        class RealTimeInputFrame(tk.Frame):
            def __init__(self, parent, controller):
                ...
                file_header.grid(
                    row=0, column=0, ipady=5, pady=5, ipadx=5, padx=5,
                )
                ...
                RegressionFrame.grid(
                    row=2, column=0, columnspan=4, pady=5, padx=5, ipadx=3
                )
                ...
                self.ShortParameterFrame = tk.Frame(
                    RegressionFrame, relief="groove", bd=2
                )  # Frame to hold the parameters for frequencies <= 50Hz
                self.ShortParameterFrame.grid(row=1, column=0, columnspan=4)
                self.LongParameterFrame = tk.Frame(
                    RegressionFrame, relief="groove", bd=2
                )  # Frame to hold the parameters for frequencies > 50Hz
                ...
                self.SelectShortParameters = ttk.Button(
                    RegressionFrame,
                    text="Short Parameters",
                    style="Off.TButton",
                    command=lambda: self.SwitchParameterFrame("Short"),
                )
                self.SelectShortParameters.grid(
                    row=2, column=0, pady=5, padx=5, sticky="nsew"
                )
                self.SelectLongParameters = ttk.Button(
                    RegressionFrame,
                    text="Long Parameters",
                    style="On.TButton",
                    command=lambda: self.SwitchParameterFrame("Long"),
                )
                self.SelectLongParameters.grid(
                    row=2, column=1, pady=5, padx=5, sticky="nsew"
                )
                ...
                ##################################################################################
                # --- Title ---#
                self.ShortLabel = tk.Label(
                    self.ShortParameterFrame,
                    text="Short Pulse Parameters",
                    font=MEDIUM_FONT,
                ).grid(row=0, column=0, columnspan=2)

                # --- points discarded at the beginning of pulse ---#
                self.short_aa_label = tk.Label(
                    self.ShortParameterFrame, text="short_aa (s)", font=MEDIUM_FONT
                ).grid(row=1, column=0)
                ...
                self.short_zz_label = tk.Label(
                    self.ShortParameterFrame, text="short_zz (s)", font=MEDIUM_FONT
                ).grid(row=1, column=1)
                ...
                self.LongLabel = tk.Label(
                    self.LongParameterFrame,
                    text="Long Pulse Parameters",
                    font=MEDIUM_FONT,
                ).grid(row=0, column=0, columnspan=2)

                # --- points discarded at the beginning of the pulse ---#
                self.long_aa_label = tk.Label(
                    self.LongParameterFrame, text="long_aa (s)", font=MEDIUM_FONT
                ).grid(row=1, column=0)
                ...
                # --- points discarded at the end of the pulse ---#
                self.long_zz_label = tk.Label(
                    self.LongParameterFrame, text="long_zz (s)", font=MEDIUM_FONT
                ).grid(row=1, column=1)
                ...
                self.AdjustParameterButton.grid(
                    row=3, column=0, columnspan=4, pady=10, padx=10
                )
                ...
                self.Exponential_LowLimit = tk.Entry(
                ...
                self.StartButton = tk.Button(
                    ...
                    command=lambda: self.StartProgram(),
                ).grid(row=4, column=0, pady=5)
                self.ResetButton = tk.Button(
                    self, text="Reset", command=lambda: self.Reset()
            ...
            def AdjustParameters(self):
                ...
                short_aa = float(
                    self.short_aa_entry.get()
                )  # aa/zz adjust the points at the start and end of the polynomial fit, respectively
...
class ElectrochemicalAnimation:
    def __init__(
        self, fig, ax, generator=None, func=None, resize_interval=None, fargs=None
    ):
        global PoisonPill
    ...
    def _step(self):
        ...
        try:
            mydata_bytes_short = os.path.getsize(
                short_file
            )  ### retrieves the size of the file in bytes
            mydata_bytes_long = os.path.getsize(
                long_file
            )  ### retrieves the size of the file in bytes
            print("Found FILES")
            mydata_bytes = True
...
class ChronoamperometryAnalysis:
    def __init__(self):
        ...
        anim = []
        self.anim = ElectrochemicalAnimation(
            figures["figure"], figures["axes"], func=self.animate, generator=self.frames
        )
    ...
    def prepare_figure(self):
        ...
        (self.fitted_lifetime,) = ax[1].plot(
            0, 0, " bo", MarkerSize=1
        )  # fitted lifetime (ms)
            ...
            with open(myfile, "r", encoding="utf-8") as mydata:
                ...
                for line in mydata:
                    ...
                    if line.startswith("VSTEP1"):
                        ...
                        voltage_1 = voltage_1[
                            2
                        ]  # and grab the potential from the 3rd column
                        volt_1 = voltage_1
    ...
    def set_fit(self, time_data, current_data):
        ...
        (
            ...
            adjusted_time,
            adjusted_currents,
        ) = self.extract_parameters(time_data, current_data)

        print(A1, C1, K1, A2, C2, K2)
        print("Initial Constant value:", C2)

        popt, pcov = curve_fit(
            self.biexponential_func,
            adjusted_time,
            adjusted_currents,
            p0=(A1, K1, A2, K2),
        )
    ...
    def extract_parameters(self, time_data, current_data):
        ...
        initial_K = (
            math.log(initial_A) - math.log(closest_current - initial_C)
        ) / closest_time
    ...
    def animate(self, framedata):
        ...
        fitted_data = []
        for x in time_data:
            fit = self.biexponential_func(
                x, *popt, e=C2
            )  # use the curve fit parameters
        ...
        self.fitted_lifetime.set_data(
            file_list, self.fitted_lifetime_list
        )  # checking curve fit
    ...
    def merge_two_dicts(
        self, x, y
    ):  # any keys that overlap will be overridden with the items from y
