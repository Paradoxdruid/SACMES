...
class Controller:
    ...
    def _retrieve_file(self, file, electrode, handle_variable, extension, mypath):
        ...
        try:
            ...
            if self.e_var == "single":
                ...
                filename3 = "E%s_%s_%d%s" % (
                    electrode,
                    handle_variable,
                    file,
                    extension,
                )
                filename4 = "E%s_%s__%d%s" % (
                    electrode,
                    handle_variable,
                    file,
                    extension,
                )
                filename5 = "E%s_%s_#%d%s" % (
                    electrode,
                    handle_variable,
                    file,
                    extension,
                )
                filename6 = "E%s_%s__#%d%s" % (
                    electrode,
                    handle_variable,
                    file,
                    extension,
    ...
    def set_fit(self, time, currents, spacer=None):
        try:
            ...
            (
                ...
                adjusted_currents,
                adjusted_dict,
            ) = self.exponential_adjustment(time, currents, spacer=spacer)
            try:
                # -- find the optimized parameters for exponential decay --#
                popt, pcov, C = self.exponential_analysis(
                    adjusted_time, adjusted_currents
                )

            except:
                try:
                    # -- if fail, call the other exponential analysis method --#
                    popt, pcov, C = self.alternate_analysis(
                        adjusted_time, adjusted_currents
                    )
    ...
    def exponential_adjustment(self, time_data, current_data, spacer=None):
        ...
        try:
            try:
                adjusted_time = [
                    time for time in time_data if self.exp_low <= time <= self.exp_high
                ]
            except:
            ...
            adjusted_dict = {
                current: time
                for (time, current) in zip(adjusted_time, adjusted_currents)
            }
    ...
    class biexponential_analysis:
        ...
        def __call__(self, time, currents):
            try:
                ...
                popt, pcov = curve_fit(self.func, time, currents, p0=(A, K1, B, K2))
    ...
    def _get_list_val(self, electrode):
        ...
        try:
            if self.e_var == "single":
                list_val = (
                    self.current_column_index + (electrode - 1) * self.spacing_index
                )
...
class MainWindow:
    ...
    def _create_toolbar(self):
        ...
        self.controller.menubar.add_cascade(
            label="Settings", menu=self.controller.editmenu
        )
        ...
        self.controller.editmenu.add_command(
        ...
        self.controller.menubar.add_cascade(
            label="Pulse Settings", menu=self.pulse_menu
        )

        # -- Average Multiple Pulses --#
        self.controller.pulse_averaging = tk.BooleanVar()
        self.controller.pulse_averaging.set(False)
        self.average_pulses = self.pulse_menu.add_checkbutton(
            label="Average Pulses",
            onvalue=True,
            offvalue=False,
            variable=self.controller.pulse_averaging,
        )

        # -- Average Multiple Steps --#
        self.controller.step_averaging = tk.BooleanVar()
        self.controller.step_averaging.set(False)
        self.average_steps = self.pulse_menu.add_checkbutton(
            label="Average Steps",
            onvalue=True,
            offvalue=False,
            variable=self.controller.step_averaging,
        )
        ...
        self.pulse_menu.add_cascade(label="Step Analysis", menu=self.step_analysis)
        self.forward_step = self.step_analysis.add_command(
            label="✓ Forward", command=lambda: self.set_step_analysis("Forward", 0)
        )
        self.reverse_step = self.step_analysis.add_command(
            label="   Reverse", command=lambda: self.set_step_analysis("Reverse", 1)
        )
        self.both_steps = self.step_analysis.add_command(
            label="   Both", command=lambda: self.set_step_analysis("Both", 2)
        )
    ...
    def set_bytes(self, bytes, index):
        ...
        self.byte_menu.entryconfigure(
            self.controller.byte_index, label="   %s" % str(self.controller.byte_limit)
        )
    ...
    def extraction_adjustment_frame(self):
        ...
        local_container.grid(
            row=row_value, column=0, columnspan=2, padx=5, pady=5, ipadx=3
        )
        ...
        l = tk.Label(
            local_container,
            text="Multipotentiostat Settings\nSpace Between Current Columns:",
        )
        ...
        self.space_delimiter = tk.Radiobutton(
            box, text="Space", variable=self.controller.delimiter_value, value=1
        )
        self.space_delimiter.grid(row=box_value, column=0, pady=5)

        box_value += 1
        self.tab_delimiter = tk.Radiobutton(
            box, text="Tab", variable=self.controller.delimiter_value, value=2
        )
        self.tab_delimiter.grid(row=box_value, column=0, pady=3)

        box_value += 1
        self.tab_delimiter = tk.Radiobutton(
            box, text="Comma", variable=self.controller.delimiter_value, value=3
        )
        self.tab_delimiter.grid(row=box_value, column=0, pady=3)
        ...
        self.txt_value = tk.Radiobutton(
            box, text="txt", variable=self.controller.extension_value, value=1
        )
        self.txt_value.grid(row=box_value, column=1, pady=5)

        box_value += 1
        self.csv_value = tk.Radiobutton(
            box, text="csv", variable=self.controller.extension_value, value=2
        )
        self.csv_value.grid(row=box_value, column=1, pady=3)

        box_value += 1
        self.dta_value = tk.Radiobutton(
            box, text="dta", variable=self.controller.extension_value, value=3
        )
        self.dta_value.grid(row=box_value, column=1, pady=3)

        row_value += 1
        apply_list_val = ttk.Button(
            win, text="Apply", command=lambda: self.get_extraction_adjustments()
        )
    ...
    def set_step_analysis(self, step, index):
        ...
        self.step_analysis.entryconfigure(
            self.controller.step_index, label="   %s" % str(self.controller.step_str)
        )
...
class InputFrame(tk.Frame):
    def __init__(self, parent, master, controller):
        ...
        self.SelectFirstFilePath = ttk.Button(
            self,
            style="Off.TButton",
        ...
        self.ImportFirstFileLabel = tk.Label(
            self, text="File Handle", font=LARGE_FONT
        ...
        self.AddSecondPulse = ttk.Button(
            self,
            style="Off.TButton",
        ...
        self.SelectSecondFilePath = ttk.Button(
            self,
            style="Off.TButton",
        ...
        self.ImportSecondFileLabel = tk.Label(
            self, text="File Handle 2", font=LARGE_FONT
        )
        ...
        self.sample_interval_label = tk.Label(
            self, text="Sample Interval (ms)", font=LARGE_FONT
        ...
        self.pulse_width_label = tk.Label(
            self, text="Pulse Width (s)", font=LARGE_FONT
        ...
        start_button = ttk.Button(
            self, text="Initialize", command=lambda: self.CheckPoint()
        )
        ...
        self.SaveBox = Checkbutton(
            ...
            offvalue=False,
            text="Export Data",
        ).grid(row=row_value, column=0, columnspan=3)
        row_value += 1
    ...
    def FindFile(self, parent, value):
        ...
        try:
            ...
            if value == 1:
                ...
                self.controller.ExportFilePath = "".join(
                    ExportFilePath + self.controller.ExportFileHandle
                )
    ...
    def ElectrodeCurSelect(self, evt):
        ...
        self.controller.electrode_list = [
            self.ElectrodeCount.get(idx) for idx in self.ElectrodeCount.curselection()
        ]
        self.controller.electrode_list = [
            int(electrode) for electrode in self.controller.electrode_list
        ]
    ...
    def CheckPoint(self):
        ...
        for value in self.controller.PathSelection:
            self.controller.PathSelection[value]["handle"] = str(
                self.controller.PathSelection[value]["input"].get()
            )
        ...
        self.controller.pulse_length = self.controller.pulse_width / (
            self.controller.sample_interval / 1000
        )  # milliseconds
        ...
        if value == 1:
            ...
            self.controller.ExportFilePath = "".join(
                self.controller.ExportFilePath + self.controller.ExportFileHandle
            )
...
class Verification:
    def __init__(self, parent, master, controller):
        ...
        title = tk.Label(self.win, text="Searching for files...", font=HUGE_FONT).grid(
            row=0, column=1, columnspan=2, pady=10, padx=10, sticky="news"
        )
        ...
        for value in self.controller.PathSelection:
            pulse_label = tk.Label(
                self.win, text=self.controller.PathSelection[value]["handle"]
            )
            ...
            for electrode in self.controller.electrode_list:
                ...
                electrode_label = tk.Label(
                    self.win, text="E%s" % electrode, fg="red", font=LARGE_FONT
                )
        ...
        self.analysis_limit = self.controller.electrode_count * len(
            self.controller.PathSelection
        )
    ...
    def verify(self, value):
        ...
        #######################################################
        myfile, mydata_bytes = self.controller._retrieve_file(
            1,
            self.electrode,
        ...
        if mydata_bytes > self.controller.byte_limit:
            ...
            if (
                self.pulse_verification[value]["count"]
                == self.controller.electrode_count
            ):
    ...
    def verify_multi(self, myfile):
        ...
        if check_:
            list_val = (
                self.controller.current_column
                + (self.electrode - 1) * self.controller.spacing_index
            )
    ...
    def proceed(self):
        ...
        self.controller.PlotContainer = tk.Frame(
            self.controller.container, relief="groove", bd=3
        )
        ...
        ############################################################
        frame = SetupVisualizationFrame(
            self.controller.PlotContainer, self.controller, self.master
        )
        ...
        self.controller.show_frame(
            SetupFrame
        )  # raises the frame for initial parameter setup
        self.controller.show_frame(
            SetupVisualizationFrame
        )  # raises the setup visualization frame
...
class VisualizationFrame(tk.Frame):
    def __init__(self, parent, electrode, figure):
        ...
        ElectrodeLabel = tk.Label(self, text="Electrode %d" % electrode, font=HUGE_FONT)
...
class SetupFrame(tk.Frame):
    def __init__(self, parent, controller, master):
        ...
        # -- Title --#
        self.Title = tk.Label(
            self, text="Initial Parameter Setup", font=LARGE_FONT
        ).grid(row=row_value, column=0, columnspan=2, pady=10, sticky="news")

        row_value += 1

        # -- ability to alter the export path of the analyzed data --#
        if self.controller.text_file_export:
            self.AlterExportPathButton = ttk.Button(
                self,
                text="Alter Export Path",
                style="Off.TButton",
                command=lambda: self.AlterExportPath(),
            )
            self.AlterExportPathButton.grid(
                row=row_value, column=0, columnspan=2, pady=10, sticky="news"
            )
            row_value += 1
        ...
        self.RegressionLabel = tk.Label(
            self.RegressionFrame, text="Regression Analysis Parameters", font=LARGE_FONT
        )
        self.RegressionLabel.grid(
            row=0, column=0, columnspan=2, pady=5, padx=5, sticky="news"
        )
        ...
        if len(self.controller.PathSelection) == 2:
            ...
            self.SecondParameterFrame = tk.Frame(
                self.RegressionFrame, relief="groove", bd=2
            )
        ...
        for value in self.controller.PathSelection:
            ...
            # --- Title ---#
            self.PulseLabel = tk.Label(
                frame, text="Pulse Parameters", font=MEDIUM_FONT
            ).grid(row=0, column=0, columnspan=2)

            xstart_label = tk.Label(frame, text="xstart (s)", font=MEDIUM_FONT).grid(
                row=1, column=0
            )
            ...
            xend_label = tk.Label(frame, text="xend (s)", font=MEDIUM_FONT).grid(
                row=1, column=1
            )
        ...
        #################################################################
        self.ExponentialFitParameterFrame = tk.Frame(
            self, bd=5, relief="groove"
        )  # container
        self.ExponentialFitParameterFrame.grid(
            row=row_value, column=0, columnspan=2, pady=5
        )
        row_value += 1

        self.ExponentialFrameLabel = tk.Label(
            self.ExponentialFitParameterFrame, text="Curve Fit Range"
        ).grid(row=0, column=0, columnspan=2, pady=5, sticky="news")

        self.Exponential_LowLabel = tk.Label(
            self.ExponentialFitParameterFrame, text="Lower Limit (ms)", font=MEDIUM_FONT
        ...
        self.Exponential_HighLabel = tk.Label(
            self.ExponentialFitParameterFrame,
            text="Higher Limit (ms)",
            font=MEDIUM_FONT,
        ).grid(row=1, column=1, sticky="news")
        self.Exponential_HighLimit = tk.Entry(
            self.ExponentialFitParameterFrame, width=7
        )
        ...
        self.controller.exponential_analysis = self.controller.monoexponential_analysis(
            self.controller
        )
        self.controller.alternate_analysis = self.controller.biexponential_analysis(
            self.controller
        )
        self.controller.exponential_str = "monoexponential"

        self.exp_analysis_label = tk.Label(
            self.ExponentialFitParameterFrame,
            text="Exponential Decay Method",
            font=MEDIUM_FONT,
        ...
        self.controller.monoexponential = tk.Radiobutton(
            self.ExponentialFitParameterFrame,
            text="Monoexpoential",
            variable=self.controller.exponential_var,
            value=1,
            command=lambda: self.set_exponential_analysis(),
        )
        self.controller.monoexponential.grid(row=4, column=0, columnspan=2, pady=5)

        self.controller.biexponential = tk.Radiobutton(
            self.ExponentialFitParameterFrame,
            text="Biexponential",
            variable=self.controller.exponential_var,
            value=2,
        ...
        self.ApplyExponentialParameters = tk.Button(
            self.ExponentialFitParameterFrame,
            text="Apply Adjustments",
            command=lambda: self.ApplyExponentialAdjustments(),
        )
        self.ApplyExponentialParameters.grid(
            row=6, column=0, columnspan=2, pady=5, sticky="news"
        )
        ...
        self.ResetButton = tk.Button(
            self, text="Reset", font=LARGE_FONT, width=7, command=lambda: self.Reset()
        )
        self.ResetButton.grid(
            row=row_value, column=0, columnspan=1, pady=5, padx=5, sticky="news"
        )
        ...
        self.Start.grid(
            row=row_value, column=1, columnspan=1, pady=5, padx=5, sticky="news"
        )
    ...
    def create_setup(self):
        ...
        self.controller.offset_label = tk.Label(
            self.RegressionFrame, text="Current Adjustment (uA)"
        )
        ...
        for value in self.controller.PathSelection:
            ...
            if mydata_bytes > self.controller.byte_limit:
                ...
                ##############################
                time_list, current_list = self.controller._read_data(
                    myfile, electrode, spacer=self.spacer
                )

                self.controller.setup_dictionary[value]["time"] = time_list
                self.controller.setup_dictionary[value]["currents"] = current_list
                self.controller.setup_dictionary[value]["dict"] = {
                    x: y for (x, y) in zip(time_list, current_list)
                }
                ...
                time, currents = self.controller._apply_adjustment(
                    time_list, current_list, value, spacer=self.spacer
                )

        # -- if there are multiple pulses (2), merge the data into a single dict --#
        if len(self.controller.PathSelection) > 1:
            data_dict = self.controller.merge_two_dicts(
                self.controller.setup_dictionary[1]["dict"],
    ...
    def AdjustParameters(self):
        try:
            ...
            for value in self.controller.PathSelection:
                ...
                self.controller.pulse_adjustments[value]["xstart"] = float(
                    self.controller.pulse_adjustments[value]["xstart entry"].get()
                )
                self.controller.pulse_adjustments[value]["xend"] = float(
                    self.controller.pulse_adjustments[value]["xend entry"].get()
                )
                ...
                time, currents = self.controller._apply_adjustment(
                    time, currents, value, spacer=self.spacer
                )
            ...
            if len(self.controller.PathSelection) > 1:
                ...
                data_dict = self.controller.merge_two_dicts(
                    local_dict[1]["dict"], local_dict[2]["dict"]
                )
            ...
            ##############################################
            exp_time, exp_currents, dict = self.controller.exponential_adjustment(
                time, currents, spacer="  "
            )

            try:
                fitted_data, fitted_lifetime = self.controller._decay_analysis(
                    time, currents, spacer="  "
                )
            ...
            if exp_print:
                ...
                with open(
                    "/Users/samdcurtis/Desktop/Merged Data.txt",
                    "w+",
                    encoding="utf-8",
                    newline="",
                ) as input:
                ...
                with open(
                    "/Users/samdcurtis/Desktop/Exponential Data.txt",
                    "w+",
                    encoding="utf-8",
                    newline="",
                ) as input:
    ...
    def set_exponential_analysis(self):
        ...
        # Biexponential
            ...
            self.controller.exponential_analysis = self.controller.biexponential_analysis(
                self.controller
            )
            self.controller.alternate_analysis = self.controller.monoexponential_analysis(
                self.controller
            )
    ...
    def AlterExportPath(self):
        ...
        try:
            ...
            self.controller.ExportFilePath = "".join(
                ExportPath + self.controller.ExportFileHandle
            )
    ...
    def Initialize(self):
        ...
        #################################################
        frame = RealTimeManipulationFrame(
            self.controller.container, self.controller, self.master
        )
        frame.grid(row=0, column=0, sticky="nsew")
...
class InitializeFigureCanvas:
    def __init__(self, master):
        ...
        for electrode in self.controller.electrode_list:
            ...
            FrameReference = VisualizationFrame(
                self.controller.PlotContainer, electrode, fig
            )
    ...
    def MakeFigure(self, electrode):
        ...
        # -- create a plot for Fitted Half life v File Number --#
        self.lifetime_plot.set_ylim(
            0.5 * fitted_lifetime[0], 1.5 * fitted_lifetime[0]
        )  # Half Life
        self.lifetime_plot.set_xlim(
            0, int(self.controller.numFiles + 0.1)
        )  # File Number
        ...
        if self.controller.exponential_str == "biexponential":
            ...
            self.lifetime_plot2.set_ylim(
                0.5 * fitted_lifetime[1], 1.5 * fitted_lifetime[1]
            )  # Half Life
            self.lifetime_plot2.set_xlim(
                0, int(self.controller.numFiles + 0.1)
            )  # File Number
    ...
    def RunInitialization(self, electrode):
        ...
        for value in self.controller.PathSelection:
            ...
            if mydata_bytes > self.controller.byte_limit:
                ...
                ##############################
                time_list, current_list = self.controller._read_data(
                    myfile, electrode, spacer=spacer
                )
                ...
                # -- to the users specifications             --#
                time, currents = self.controller._apply_adjustment(
                    time_list, current_list, value, spacer=spacer
                )

        # -- if there are multiple pulses (2), merge the data into a single dict --#
        if len(self.controller.PathSelection) > 1:
            data_dict = self.controller.merge_two_dicts(
                local_dictionary[1], local_dictionary[2]
            )
    ...
    def CreateArtists(self, ax):
        ...
        if self.controller.exponential_str == "biexponential":
            (self.fitted_lifetime2,) = self.lifetime_plot2.plot(
                0, 0, " bo", MarkerSize=1
            )
...
class RealTimeManipulationFrame(tk.Frame):
    def __init__(self, parent, controller, master):
        ...
        self.controller.FileLabel.grid(
            row=row_value, column=0, columnspan=2, ipady=5, pady=5, ipadx=5, padx=5
        )
        row_value += 1

        self.controller.FileNumber = ttk.Label(
            self, text="1", font=LARGE_FONT, style="Fun.TButton"
        )
        self.controller.FileNumber.grid(
            row=row_value, column=0, columnspan=2, padx=5, ipadx=5, sticky="news"
        )
        row_value += 1
        ...
        if len(self.controller.PathSelection) == 2:
            ...
            self.FirstParameterFrame = tk.Frame(
                self.RegressionFrame, relief="groove", bd=2
            )
            self.FirstParameterFrame.grid(row=2, column=0, columnspan=4)
            self.controller.pulse_widgets["frames"][1] = self.FirstParameterFrame
        ...
        # -- set the initial verical offset for currents as None --#
        self.controller.offset_header = tk.Label(
            self.RegressionFrame, text="Current Adjustment (uA)"
        )
        ...
        if self.controller.electrode_count > 1:
            for electrode in self.controller.electrode_list:
                Button = ttk.Button(
                    self,
                    text="E%d" % electrode,
                    command=lambda electrode=electrode: self.controller.show_frame(
                        electrode
                    ),
                )
        ...
        self.StartButton = tk.Button(
            self, text="Begin Visualization", command=lambda: self.SkeletonKey()
        )
    ...
    def SkeletonKey(self):
        ...
        if not self.controller.AlreadyInitiated:
            ...
            for electrode in self.controller.electrode_list:
                ...
                anim[electrode] = ElectrochemicalAnimation(
                    self.master,
                    self.controller,
                    generator,
                    func,
                    electrode,
                    self.controller.figures[electrode]["figure"],
                    frames=self.controller.numFiles,
                    init_func=generator.init_func,
                )
...
class ElectrochemicalAnimation:
    ...
    def _start(self, *args):
        ...
        threaded_animation = _threaded_animation(
            self, self.master, self.controller, self.Interval, frames=self.frames
        )
...
class CA_Generator:
    def __init__(self, electrode, master, fargs=None):
        ...
        if self.controller.exponential_str == "biexponential":
            self.fitted_lifetime2 = self.controller.artist_list[electrode][
                "fitted lifetime 2"
            ]
    ...
    def __call__(self):
        ...
        if self.file <= self.controller.numFiles:
            ...
            myfile, mydata_bytes = self.controller._retrieve_file(
                self.file,
                self.electrode,
                self.controller.PathSelection[self.path_index]["handle"],
                self.controller.extension,
                self.controller.PathSelection[self.path_index]["mypath"],
            )
            ...
            if mydata_bytes > self.controller.byte_limit:
                ...
                self.local_dict[self.path_index]["dict"] = {
                    x: y for (x, y) in zip(time, currents)
                }
            ...
            if self.path_count == self.path_number:
                ...
                if self.path_number > 1:
                    data_dict = self.controller.merge_two_dicts(
                        self.local_dict[1]["dict"], self.local_dict[2]["dict"]
                    )
                ...
                ##############################################
                fitted_data, fitted_lifetime = self.controller._decay_analysis(
                    time, currents, spacer=self.spacer
                )
    ...
    def _generator(self, myfile, value):
        ...
        try:
            ...
            print(
                ...
                % (self.spacer, self.electrode, self.file)
            )

            ##############################
            ### Get data from the file ###
            ##############################
            time, currents = self.controller._read_data(
                myfile, self.electrode, spacer=self.spacer
            )
    ...
    def animate(self, framedata):
        ...
        try:
            ...
            self.curve_fit.set_data(
                time_data, fitted_data
            )  # set the curve fit as an artist
            animation_list.append(self.curve_fit)

            # set the half life plots
            file_list = range(1, self.file + 1)
            if self.controller.exponential_str == "monoexponential":
                self.fitted_lifetime.set_data(
                    file_list, self.controller.fitted_lifetime_list[self.num]
                )
                ...
                fitted_lifetime_one = [
                    x[0] for x in self.controller.fitted_lifetime_list[self.num]
                ]
                fitted_lifetime_two = [
                    x[1] for x in self.controller.fitted_lifetime_list[self.num]
                ]
...
class TextFileExport:
    ...
    def __init__(self, master):
        ...
        try:
            ...
            # --- Write the data into the .txt file ---#
            with open(
                self.controller.ExportFilePath, "w+", encoding="utf-8", newline=""
            ) as input:
                writer = csv.writer(input, delimiter=" ")
                writer.writerow(list)
    ...
    def RealTimeExport(self, file):
        ...
        # --- Write the data into the .txt file ---#
        with open(
            self.controller.ExportFilePath, "a", encoding="utf-8", newline=""
        ) as input:
            writer = csv.writer(input, delimiter=" ")
            writer.writerow(list)
        with open(
            self.controller.ExportFilePath, "r", encoding="utf-8", newline=""
        ) as filecontents:
